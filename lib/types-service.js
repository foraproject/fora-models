(function() {
	...
	TypesService.prototype.completeVirtualTypeDefinition = function(virtualTypeDef, ctor) {
		var baseTypeDef = ctor.getTypeDefinition();

		var def = utils.clone(baseTypeDef);
		...
		def.schema.properties = {};
		for (k in virtualTypeDef.schema.properties) {
			def.schema.properties[k] = virtualTypeDef.schema.properties[k];
		}

		for (k in baseTypeDef.schema.properties) {
			def.schema.properties[k] = baseTypeDef.schema.properties[k];
		}
	...
	TypesService.prototype.buildTypeCache = function*(ctors) {
		var def, type;

		//Add the base types
		ctors.forEach(function(ctor) {
			def = this.completeTypeDefinition(baseType.typeDefinition, ctor);
			typeCache[def.name] = def;
		});

		//Add virtual types
		if (this.getVirtualTypeDefinitions) {
			var virtTypeList = yield* this.getVirtualTypeDefinitions();
			virtTypeList.forEach(function(item) {
				def = this.completeVirtualTypeDefinition(item.typeDefinition, item.ctor);
				typeCache[def.name] = def;
			});
		}
	...
	TypesService.prototype.getTypeDefinition = function*(name, dynamicResolutionContext) {
		return typeCache[name] || dynamicResolutionContext[name] || (yield* this.getDynamicTypeDefinition(name, dynamicResolutionContext));
	};
