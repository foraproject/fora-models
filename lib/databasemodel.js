(function() {
	...
	DatabaseModel.find = function*(query, options, context) {
		...
		if (items.length) {
			...
			for (var _i = 0; _i < items.length; _i++) {
				var item = items[_i];
				results.push(yield* typesService.constructModel(item, typeDefinition, context));
			}
			return results;
		} else {
			return [];
	...
	DatabaseModel.findOne = function*(query) {
		...
		if (result) {
			return yield* typesService.constructModel(result, typeDefinition, context);
		}
	};
	...
	DatabaseModel.prototype.save = function*(context) {
		...
		if (!errors.length) {
			...
			if (!db.getRowId(this)) {
				...
				result = yield* typesService.constructModel(result, typeDefinition, context);
			} else {
